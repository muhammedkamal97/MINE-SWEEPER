...
void* timer(void* p){
    ...
    while(x==0){
        Sleep(100);
        i++;
        if(i%200==0){printing(nx,mx,v);} //wait 20 seconds then print the grid with the time update
...
int getInt(int vx,int lim){
    ...
    } while (!isspace(*chk) && *chk != 0 || value>=lim || value<0 || i==1||c>=strlen(buffer));
